"""Find correct Google Flights city IDs using keyboard-driven search."""
import sys, time, re, base64
sys.stdout.reconfigure(encoding='utf-8', errors='replace')
from playwright.sync_api import sync_playwright

CITIES_TO_FIND = ['Phoenix', 'Baltimore', 'Philadelphia', 'Portland',
                   'Salt Lake City', 'San Diego', 'Orlando', 'Savannah']

def extract_dest_id(url):
    """Extract destination city ID from TFS in URL."""
    if 'tfs=' not in url:
        return None
    ...
    try:
        ...
        return None
    # Find all /m/ patterns in the raw bytes
    text = raw.decode('utf-8', errors='replace')
    ids = re.findall(r'/m/[a-z0-9_]+', text)
    # The destination is typically the 2nd unique /m/ ID
    seen = []
    for mid in ids:
        if mid not in seen:
            seen.append(mid)
    # For Taipei->X search: first is /m/0ftkx (Taipei), second should be dest
    if len(seen) >= 2:
        return seen[1]
    return None
...
with sync_playwright() as p:
    ...
    for city in CITIES_TO_FIND:
        ...
        try:
            # Try to find and click the "Where from?" input
            inputs = page.locator('input[type="text"], input[aria-label*="here"]')
            count = inputs.count()

            # Try clicking the first combobox/input area
            ...
            # Check URL for dest ID
            url = page.url
            dest_id = extract_dest_id(url)
            if dest_id:
                print(f"  '{city}': '{dest_id}',")
            else:
                # Try getting title to see what page we're on
                title = page.title()
